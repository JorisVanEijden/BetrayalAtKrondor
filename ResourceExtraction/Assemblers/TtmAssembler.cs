...
public class TtmAssembler {
    ...
    public static void Assemble(AnimationResource animationResource, string filePath) {
        ...
        // Write VER tag
        WriteTag(writer, "VER");
        var versionBytes = Encoding.ASCII.GetBytes(animationResource.Version);
        ...
        writer.Write((ushort)0); // Unknown
        writer.Write(animationResource.Frames.Count);

        // Write TT3 tag
        WriteTag(writer, "TT3");
        var scriptData = ConstructScriptData(animationResource);
        ...
        writer.Write((ushort)0); // Unknown
        writer.Write((ushort)animationResource.Tags.Count); // Number of tags

        foreach (var tagEntry in animationResource.Tags) {
    ...
    public static byte[] ConstructScriptData(AnimationResource animationResource) {
        using var memoryStream = new MemoryStream();
        using var writer = new BinaryWriter(memoryStream, Encoding.UTF8, true);

        foreach (var frame in animationResource.Frames) {
